...
def _camera_variant_path(manifest_path: Path, entry: dict) -> str | None:
    """Resolve camera assets generated by older manifests.

    Early manifests recorded the canonical LIBERO BDDL for camera variants,
    which silently disabled the filename based camera perturbation.  The
    generated per-variant file is preferred when it is present so existing
    manifests become correct after upgrading RLinf.
    """
    if entry.get("category") != "camera":
        return None
    params = entry.get("parameters", {})
    values = (
        params.get("horizon_view"),
        params.get("vertical_view"),
        params.get("scale_factor"),
        params.get("end_point_rot"),
        params.get("end_point_vertical"),
    )
    if any(value is None for value in values):
        return None
    scale = int(values[2])
    filename = (
        f"{entry['base_task_name']}_view_{int(values[0])}_{int(values[1])}_"
        f"{scale}_{int(values[3])}_{int(values[4])}_initstate_0.bddl"
    )
    candidate = manifest_path.parent / "bddl" / "camera" / entry["severity"] / filename
...
class LIBERO_90_PLUS_OOD:
    ...
    def __init__(self, task_order_index: int = 0):
        ...
        for entry in manifest.get("tasks", []):
            bddl_file = _camera_variant_path(manifest_path, entry)
            task = SimpleNamespace(
                name=entry["id"],
                language=entry["language"],
                problem="Libero",
                problem_folder="libero_90_plus_ood",
                bddl_file=bddl_file or entry["bddl_path"],
